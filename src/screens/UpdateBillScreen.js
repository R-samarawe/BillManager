...
const UpdateBillScreen = ({ navigation, route }) => {
  // bill object
  /**
 1. id 
 2. duedate
 ...
 */
  ...
  const clickedItem = route.params.id; // will give document id

  const [billingList, setBillingList] = useState([]);
  const readBillItem = async () => {
    try {
      const collecRef = collection(fireStore, "bills");
      //const q = query(collecRef, where("userId", "==", clickedItem));
      const querySnapshot = await getDocs(collecRef, clickedItem);
      console.log("Reading " + clickedItem);
      setBillingList(
  ...
  const updateBillItem = async () => {
    try {
      ...
      const docRef = doc(fireStore, "bills", clickedItem);
    ...
    <SafeAreaView style={styles.container}>
      <View>
        ...
        <View style={{ alignItems: "center" }}>
          <Pressable style={styles.submitButton} onPress={updateBillItem}>
